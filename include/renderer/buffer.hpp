...
class Buffer {
 public:
  Buffer(Renderer& renderer, std::span<const T> data, wgpu::BufferUsage usage)
  ...
  Buffer(Renderer& renderer, std::size_t size, wgpu::BufferUsage usage)
 ...
 private:
  ...
  Buffer(Renderer& renderer, std::size_t size, wgpu::BufferUsage usage, bool mappedAtCreation) {
